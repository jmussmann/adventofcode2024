...
def count_unique_pos(positions):
    
    pos_map = {}
    res = 0
    for pos in positions:
        if pos[0] not in pos_map:
            pos_map[pos[0]] = {}
        if pos[1] not in pos_map[pos[0]]:
...
def process_field(field):
    ...
    for y_pos, line in enumerate(field):
        ...
        for zero_x_pos in zeros:
            reachable = step(zero_x_pos, y_pos, 0, field)
            res += count_unique_pos(reachable)
    return res
