    ...
    if operator == "*":
        ...
        if res != result:
            ...
            if res != result:
                res = process_line(result, numbers[1:], "||", in_res * numbers[0])
    if operator == "||":
        res = process_line(result, numbers[1:], "+", int(str(in_res) + str(numbers[0])))
        if res != result:
            res = process_line(result, numbers[1:], "*", int(str(in_res) + str(numbers[0])))
            if res != result:
                res = process_line(result, numbers[1:], "||", int(str(in_res) + str(numbers[0])))
...
def process_lines(results, numbers):
    ...
    for result, nums in zip(results, numbers):
        ...
        if process_line(result, nums, "+", 0) == result or process_line(result, nums, "*", 1) == result or process_line(result, nums, "||", 0) == result:
            res += result
